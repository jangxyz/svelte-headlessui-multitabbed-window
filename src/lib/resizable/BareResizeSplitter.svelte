<script lang="ts">
  ...
  export let domain: HTMLElement | undefined = undefined;

  export let limitY: ((ev: MouseEvent) => boolean | undefined) | undefined = undefined;
  export let limitX: ((ev: MouseEvent) => boolean | undefined) | undefined = undefined;

  const DEFAULT_GET_CLASS = () =>
    resolveClasses(
      ['splitter', direction],
  ...
  let classes: string | ((defaultValue: string) => string) = DEFAULT_GET_CLASS;
  export { classes as class };

  const DEFAULT_CENTERLINE_CLASS = 'center-line relative';
  export let centerLineClass: string | ((defaultValue: string) => string) = DEFAULT_CENTERLINE_CLASS;
